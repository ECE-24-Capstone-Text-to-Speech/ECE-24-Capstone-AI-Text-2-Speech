...
from tortoise_tts.tortoise.models.diffusion_decoder import DiffusionTts
from tortoise_tts.tortoise.models.autoregressive import UnifiedVoice
from tqdm import tqdm
from tortoise_tts.tortoise.models.arch_util import TorchMelSpectrogram
from tortoise_tts.tortoise.models.clvp import CLVP
from tortoise_tts.tortoise.models.cvvp import CVVP
from tortoise_tts.tortoise.models.random_latent_generator import RandomLatentConverter
from tortoise_tts.tortoise.models.vocoder import UnivNetGenerator
from tortoise_tts.tortoise.utils.audio import (
    wav_to_univnet_mel,
    denormalize_tacotron_mel,
    TacotronSTFT,
)
from tortoise_tts.tortoise.utils.diffusion import (
    SpacedDiffusion,
    space_timesteps,
    get_named_beta_schedule,
)
from tortoise_tts.tortoise.utils.tokenizer import VoiceBpeTokenizer
from tortoise_tts.tortoise.utils.wav2vec_alignment import Wav2VecAlignment
    ...
    def get_conditioning_latents(self, voice_samples, return_mels=False):
        ...
        with torch.no_grad():
            ...
            if self.stft is None:
                # Initialize STFT
                self.stft = TacotronSTFT(1024, 256, 1024, 100, 24000, 0, 12000).to(
                    self.device
                )
    ...
    def tts(
        ...
        **hf_generate_kwargs,
        ...
        if voice_samples is not None:
            auto_conditioning, diffusion_conditioning, auto_conds, _ = (
                self.get_conditioning_latents(voice_samples, return_mels=True)
            )
        elif conditioning_latents is not None:
            auto_conditioning, diffusion_conditioning = conditioning_latents
        else:
            auto_conditioning, diffusion_conditioning = (
        ...
        with torch.no_grad():
            ...
            if not torch.backends.mps.is_available():
                with self.temporary_cuda(self.clvp) as clvp, torch.autocast(
                    device_type=(
                        "cuda" if not torch.backends.mps.is_available() else "mps"
                    ),
                    dtype=torch.float16,
                    enabled=self.half,
                ):
            ...
            if not torch.backends.mps.is_available():
                with self.temporary_cuda(
                    self.diffusion
                ) as diffusion, self.temporary_cuda(self.vocoder) as vocoder:
